...
TEST(stack_format, profile_printf)
{
	for (int i = 0; i < profile_count; ++i)
	{
		char buffer[1024];
		//snprintf(buffer, 1024, "[%d, %d, %d]", i, i * 2, i / 2);
		//snprintf(buffer, 1024, "[%f, %f, %f]", static_cast<float>(i), i * 2.0f, i / 2.0f);
		snprintf(buffer, 1024, "[%s, %s, %s]", "foo", "bar", "baz");
	}
}
TEST(stack_format, profile_stringstream)
{
	for (int i = 0; i < profile_count; ++i)
	{
